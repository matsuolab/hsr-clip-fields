...
class ClipFields:
    def __init__(self):
        rospy.init_node("clip_fields_pred", anonymous=True)
        self.model, preprocess = clip.load("ViT-B/32", device=DEVICE)
        self.sentence_model = SentenceTransformer("all-mpnet-base-v2")
        self.training_data = torch.load("/root/catkin_ws/src/ros_docker/hsr_collection/scripts/hsr-clip-fields/detic_labeled_dataset_hsr.pt")
        ...
        model_weights_path = "/root/catkin_ws/src/ros_docker/hsr_collection/scripts/hsr-clip-fields/clip_implicit_model/implicit_scene_label_model_latest_hsr.pt"
        ...
        # service
        self.pred_service = rospy.Service("cf_pred", CFPred, self.pred)
    ...
    def pred(self, req):
        ...
        queries = [req.query]
        alignment_q = self.find_alignment_over_model(self.label_model, queries, self.points_dataloader, visual=False)
        ...
        for query, q in zip(queries, alignment_q):
            ...
            if use_threshold:
                ...
                results.append(max_point.tolist())
            else:
                points = self.training_data._label_xyz
                colors = all_colors
                results.append(pts[torch.argmax(a_norm)].tolist())

        print(f"Time taken: {time.time() - start_time}")
        print(results)

        res = CFPredResponse()
        res.coordinate = results[0]
        return res
