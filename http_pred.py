...
class ClipFields:
    ...
    def pred(self, queries: str, visual:bool=False, use_threshold:bool=True)->Dict[str, List[float]]:
        queries = [queries]
...
if __name__ == '__main__':
    ...
    cf = ClipFields()
    # {"query":srt}を受け取って、queryに代入しその文字列に対応する座標（リスト）resを返す
    @post('/cf_pred')
    def cf_pred():
        print("cf_pred")
        start_time = time.time()
        query = json.loads(request.body.read().decode('utf-8'))['query']
        print(query)
        res = cf.pred(query, visual=False, use_threshold=True)
